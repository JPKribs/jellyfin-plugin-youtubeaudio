...
public class ConfigurationController : ControllerBase
{
    ...
    public ActionResult<MetadataSyncItemDto> GetMetadataSyncItem(long id)
    {
        ...
        return Ok(MapToMetadataSyncItemDto(item, plugin.Configuration.LibraryMappings));
    }
    ...
    public ActionResult<MetadataSyncStatusResponse> GetMetadataSyncStatus()
    {
        ...
        var counts = plugin.Database.GetMetadataSyncStatusCounts();
        var libraryMappings = plugin.Configuration.LibraryMappings ?? new List<LibraryMapping>();
        ...
        return Ok(new MetadataSyncStatusResponse
        {
            ...
            Errored = counts.GetValueOrDefault(BaseSyncStatus.Errored, 0),
            Ignored = counts.GetValueOrDefault(BaseSyncStatus.Ignored, 0),
            LastSyncTime = plugin.Configuration.LastMetadataSyncTime,
    ...
    public ActionResult UpdateMetadataSyncItemStatus([FromBody] UpdateMetadataSyncItemStatusRequest request)
    {
        ...
        plugin.Database.UpdateMetadataSyncItemStatusById(request.Id, status);
        return Ok(new { Success = true });
    }
    ...
    public ActionResult QueueMetadataSyncItems([FromBody] BulkMetadataSyncItemsRequest request)
    {
        ...
        try
        {
            successCount = plugin.Database.BatchUpdateMetadataSyncItemStatusByIds(request.Ids, BaseSyncStatus.Queued);
        }
        catch (Exception ex)
        {
            plugin.LoggerFactory.CreateLogger<ConfigurationController>()
                .LogWarning(ex, "Failed to queue metadata sync items by IDs");
        }
    ...
    public ActionResult IgnoreMetadataSyncItems([FromBody] BulkMetadataSyncItemsRequest request)
    {
        ...
        try
        {
            successCount = plugin.Database.BatchUpdateMetadataSyncItemStatusByIds(request.Ids, BaseSyncStatus.Ignored);
        }
        catch (Exception ex)
        {
            plugin.LoggerFactory.CreateLogger<ConfigurationController>()
                .LogWarning(ex, "Failed to ignore metadata sync items by IDs");
        }
    ...
    public ActionResult ResetMetadataSyncDatabase()
    {
        ...
        try
        {
            plugin.Database.ResetMetadataSyncDatabase();
            logger.LogInformation("Metadata sync database has been reset");
            return Ok(new { Success = true, Message = "Metadata sync database reset successfully" });
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Failed to reset metadata sync database");
            return StatusCode(500, new { Success = false, Error = ex.Message });
        }
    }
